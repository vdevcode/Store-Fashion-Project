...
function updates_sanpham($id,$categories,$product_code,$ten_sp,$gia,$gia_giam,$img,$img1,$img2,$img3,$date,$mota,$special,$view){
    $sql = "UPDATE sanpham SET id=?, id_catalog=?, ma_sp=?, ten_sp=?, gia=?,  giam_gia=?,  hinh=?,  hinh1=?,  hinh2=?,  hinh3=?,  ngay_nhap=?,  mo_ta=?,  dac_biet=?,  so_luot_xem=? WHERE id=".$id;
    return pdo_execute($sql,$id,$categories,$product_code,$ten_sp,$gia,$gia_giam,$img,$img1,$img2,$img3,$date,$mota,$special,$view);
}
...
function showluotmua($dssp_luotmua) {
    ...
    if (is_array($dssp_luotmua) && count($dssp_luotmua) > 0) {
        foreach ($dssp_luotmua as $sp) {
            ...
            $html_dssp_luotmua .= '<div class="pro">
                                        <a href="index.php?pg=sproduct&id=' . $id . '">
                                            <img src="layout/img/products/' . $hinh . '" alt=""' . $ten_sp_gioihan . '>
                                        </a>
                                        <h6><a href="index.php?pg=sproduct&id=' . $id . '">' . $ten_sp_gioihan . '</a></h6>
